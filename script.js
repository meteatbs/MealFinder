...
//Search meal and fetch from API its submit so we have to use 'e'
...
//Fetch meal by ID
function getMealById(mealID) {/*Fetch give us promise to use it dynamic we take a response and make it json and another promise for data*/
    fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${mealID}`).then(res => res.json()).then(data=>{
        // console.log(data);
        const meal=data.meals[0];//we turn it into an array and first value of arr
...
mealsEL.addEventListener('click',e => {/*we want to return mealinfo*/
